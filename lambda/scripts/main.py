"""
    ...
    Lambda function to manage the EBS datas disk affinity for a new instance(defined by the instance id in the SNS notification).
    This function is aimed to be called by an SNS notification
    Features:
        * check if an EBS is available for this new instance.
                (An EBS available is an EBS located in the same AZ than the new instance and with the same tag than the instance).
        ...
        * If a resize is made the previous EBS volume is taggued like this "To Delete": "True"
...
def retrieve_instance_infos(instanceid):
    """ Return a dict of instance infos.
        Example: {"az": xx, "vpc_id": xxx, "private_ip": xxx, "disks_attach": ["/dev/..", "/dev/..."]}
...
def find_ebs_volume(filters, az):
    """ Return a dict of informations about the volume ID only if
        ...
        :param  filters  dict: The tag name and tag value used to filter EBS volumes.
...
def check_the_resource_state(wait_type, resource_name, resource_id,  max_retry = 120):
    """ Return True only if the resource is ready(no operation in progress).
        ...
        :param  resource_name  string: The resouce name to wait(ex:VolumeIds, SnapshotIds)
...
def create_ebs_volume(tags, az, volume_type, volume_size, iops, snap_id = None):
    """ Create a new EBS volume, tag it and returns the EBS id.
        ...
        :param  tags  dict: The tag name and tag value used to filter EBS volumes.
        ...
        :param  SnapshotId  string:  The snapshot id to use during the create operation.
        :return string:  The id of the new volume.
    """
    try:
        if snap_id:
            if iops:
                volume_id = ec2.create_volume(Size = int(volume_size), AvailabilityZone = az,
                    VolumeType = volume_type, Iops = iops, SnapshotId = snap_id)['VolumeId']
            else:
                volume_id = ec2.create_volume(Size = int(volume_size), AvailabilityZone = az,
                    VolumeType = volume_type, SnapshotId = snap_id)['VolumeId']
        else:
            if iops:
                volume_id = ec2.create_volume(Size = int(volume_size), AvailabilityZone = az,
                                          VolumeType = volume_type, Iops = iops)['VolumeId']
            else:
                volume_id = ec2.create_volume(Size = int(volume_size), AvailabilityZone = az,
                                                        VolumeType = volume_type)['VolumeId']
        if check_the_resource_state('volume_available', 'VolumeIds', volume_id):
...
def check_snapshot_exist(volume_id, time_limit):
    """ Return a snapshot id if a snapshot exists for the volume id
        in parameter and if it was created in the latest time_limit value(in minutes).

        :param  volume_id  string: The volume id to attach.
        :param  time_limit  string: The time delta limit in minutes between now and the snapshot creation date.
        :return string  The  snapshot id
    """
    snap_ids = ec2.describe_snapshots(Filters=[{'Name': 'volume-id', 'Values': [volume_id]}])['Snapshots']
    for snap in snap_ids:
        if int((datetime.datetime.now(dateutil.tz.tzutc()) - snap['StartTime']).seconds) < (time_limit * 60):
            if check_the_resource_state('snapshot_completed', 'SnapshotIds', snap["SnapshotId"]):
...
def create_snapshot_if_not_exist(volume_id, tags, time_limit):
    """ Create a snapshot of the volume in parameter only if any snapshot
        ...
        define by the difference between now and the time_limit value(in minutes).
    ...
    try:
        ...
        snapshot_id = ec2.create_snapshot(VolumeId = volume_id, Description='Snapshot create to scale the volume: {0}' .format(volume_id))['SnapshotId']
        ec2.create_tags(Resources = [snapshot_id], Tags = tags)
        logger.debug("Waiting until the snapshot become available")
        if not check_the_resource_state('snapshot_completed', 'SnapshotIds', snapshot_id):
...
def manage_ebs_volume(config, instanceid, instance_infos):
    """ Manage the whole EBS process
        ...
        :param  ebs_config  dict  The EBS parameters
            -  tags  dict: The tag name and tag value to set if a new EBS volumes is created.
            -  filters  dict: The tag name and tag value used to filter EBS volumes.
            -  volume_type  string: The type of volume to create(ex:GP2)
            -  volume_size  int: The size(GB) of the new volume to create.
            -  time_limit  string: The time delta limit in minutes between now and the snapshot creation date.
        :param instance_infos  dict {"az": xx, "vpc_id": xxx, "private_ip": xxx, "disks_attach": ["/dev/..", "/dev/..."]}
        :return Bool
    """
    ...
    if ebs_infos and int(ebs_infos['size']) == int(config['volume_size']) and ebs_infos['type'] == config['volume_type'] and int(ebs_infos['iops']) == int(config['volume_iops']):
        ...
        new_vol_infos = {"size": config['volume_size'], "type": config['volume_type'], "iops": config['volume_iops']}
        ...
        if int(new_vol_infos["size"]) < int(ebs_infos["size"]) and ebs_infos['type'] == config['volume_type']:
            ...
            return False
        snap_id = create_snapshot_if_not_exist(ebs_infos['id'], config['tags'], config['time_limit'])
        if snap_id:
            ebs_id = create_ebs_volume(config['tags'], instance_infos['az'], config['volume_type'], config['volume_size'], config['volume_iops'], snap_id)
        else:
            logger.error("Can\'t create snapshot")
            return False
    else:
        logger.debug("No volume available. Launch the creation process")
        ebs_id = create_ebs_volume(config['tags'], instance_infos['az'], config['volume_type'], config['volume_size'], config['volume_iops'])
    if ebs_id:
...
def check_if_ebs_already_attached(instanceid, mount_point, instance_infos):
    """ Check if the instance has already a disk attached on the target
        ...
        :param mount_point  string  The mount point where the EBS volume must be attached.
        :param instance_infos  dict {"az": xx, "vpc_id": xxx, "private_ip": xxx, "disks_attach": ["/dev/..", "/dev/..."]}
        :return  Bool True if a disk is already attached on the instance for this mount point.
    """
...
def load_configuration_params(parser, section_name):
    ...
    """
    filters = {'Name': "tag:{0}" .format(parser.get(section_name, 'tag_name')), "Values": [parser.get(section_name, 'tag_value')]}
    tags = [{'Key': parser.get(section_name, 'tag_name'), 'Value': parser.get(section_name, 'tag_value')}]
    params = {'volume_size': int(parser.get(section_name, 'volume_size')), 'volume_type': parser.get(section_name, 'volume_type'),
            'mount_point': parser.get(section_name, 'mount_point'), 'volume_iops' : int(parser.get(section_name, 'volume_iops')),
            'time_limit': parser.get(section_name, 'time_limit'), 'tags': tags, 'filters': filters}
    return params
...
def launch_ebs_affinity_process(instanceid, instance_infos, ebs_configs):
    """ Manage the ebs affinity process.
        ...
        :return None
    """
    if not check_if_ebs_already_attached(instanceid, ebs_configs['mount_point'], instance_infos):
